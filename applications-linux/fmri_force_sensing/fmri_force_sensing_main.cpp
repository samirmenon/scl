/* \file fmri_force_sensing_main.cpp
 *
...
int main(int argc, char** argv)
{
  ...
  else
  {
    try
    {
      ...
      if(false == flag) { throw(std::runtime_error("Could not add sphere at haptic position"));  }
      if(NULL == chai_haptic_pos){ throw(std::runtime_error("Could not add sphere at haptic position"));  }
      ...
      if(false == flag) { throw(std::runtime_error("Could not add sphere at haptic desired pos"));  }
      if(NULL == chai_haptic_pos_des){ throw(std::runtime_error("Could not add sphere at haptic desired pos"));  }
       ...
       * ONLY TWO STATES HERE */
      enum fMRIState{/** 0 */FMRI_CENTER, /** !=0 */FMRI_VISION_AND_FORCE,
        /** The remaining enum ids are for the state initialization code */
        FMRI_INCREMENT_TASK_STATE,
        FMRI_CENTER_INIT,
        FMRI_VISION_AND_FORCE_INIT,
        FMRI_EXIT};
      fMRIState state=FMRI_CENTER_INIT; //Always start off at the rest state
       ...
       * 2. Only time-at-completion is used for the timer (this simplifies code to prevent time-drift)
       * 3. Workspace : Roughly 24" (along bore) x 12" (width) x 8" (height)
       *                ==      0.6 x 0.3 x 0.2 m (xyz in chai)
       *                ==      0.3 x 0.15 x 0.1 m (xyz displacements to reach cuboid edges from center)
       */
      flag = scl_util::readEigenMatFromFile(state_task_selection_matrix, disp_filename);
      if(false == flag)
      { throw(std::runtime_error("Could not open display input file"));  }
...
#pragma omp parallel private(thread_id)
      {
        ...
        if(thread_id==1)
        {
          ...
          while(true == scl_chai_glut_interface::CChaiGlobals::getData()->chai_glut_running)
          {
            ...
            // The init states set up the next state's graphics
            // The transition is to simplify the move to the next state.
            switch(state)
            {
              case FMRI_CENTER:
              ...
              case FMRI_VISION_AND_FORCE_INIT:    // R : no, G : yes, Y : no
                //Goal boxes
                chai_haptic_box_des_red->setEnabled(false,true);
                ...
                //Switch tasks
                std::cout<<"\nSwitching to : FMRI_VISION_AND_FORCE"<<std::flush;
                state = FMRI_VISION_AND_FORCE;
                break;
