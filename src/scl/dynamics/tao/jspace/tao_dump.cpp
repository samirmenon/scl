...
std::string inertia_matrix_to_string(deMatrix3 const & mx)
{
  ...
  static void _dump_tao_tree_info(std::ostream & os, taoDNode * root, std::vector<STaoNodeInfo> const & info,
  ...
  static void _dump_tao_tree_info_saixml(std::ostream & os, taoDNode * root, std::vector<STaoNodeInfo> const & info,
				   ...
				   STaoTreeInfo * tree) throw(std::runtime_error)
  {
    ...
    for (std::vector<STaoNodeInfo>::const_iterator inode(tree->info.begin());
	...
	if (parent == tree->root) {
	  ...
	  for (std::vector<STaoNodeInfo>::const_iterator jj(tree->info.begin());
	       jj != tree->info.end(); ++jj) {
	    if (jj->node == parent) {
