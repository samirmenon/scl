...
namespace jspace {
  ...
  int Model::init(STaoTreeInfo * kgm_tree,
  ...
  void Model::update(State const & state)
  {
    setState(state);
    updateKinematics();
    updateDynamics();
  ...
  void Model::setState(State const & state)
  {
    state_ = state;
  ...
  std::string Model::getNodeName(size_t id) const
  {
    std::string name("");
    if (ndof_ > id) {
  ...
  std::string Model::getJointName(size_t id) const
  {
    std::string name("");
    if (ndof_ > id) {
  ...
  taoDNode * Model::getNode(size_t id) const
  {
    if (ndof_ > id) {
  ...
  taoDNode * Model::getNodeByName(std::string const & name) const
  {
    for (size_t ii(0); ii < ndof_; ++ii) {
  ...
  taoDNode * Model::getNodeByJointName(std::string const & name) const
  {
    for (size_t ii(0); ii < ndof_; ++ii) {
...
#warning "TO DO: maybe the other way around..."
  ...
  bool Model::disableGravityCompensation(size_t index, bool disable)
  {
    if (ndof_ <= index) {
      return true;
    }
...
#ifdef W_QNX
    dof_set_t::iterator const idof(gravity_disabled_.find(index));
#else
  ...
  void Model::computeCoriolisCentrifugal()
  {
    if (cc_tree_) {
  ...
  void Model::computeInverseMassInertia()
  {
