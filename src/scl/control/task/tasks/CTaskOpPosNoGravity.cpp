...
namespace scl
{
  ...
  // Inherited stuff
  //************************
  bool CTaskOpPosNoGravity::init(STaskBase* arg_task_data,
      CDynamicsBase* arg_dynamics)
  {
    try
    {
      ...
      data_ = dynamic_cast<STaskOpPosNoGravity*>(arg_task_data);
    ...
    {
      std::cerr<<"\nCTaskOpPosNoGravity::init() :"<<e.what();
      has_been_init_ = false;
    }
    return has_been_init_;
  }

  STaskBase* CTaskOpPosNoGravity::getTaskData()
  { return data_; }

  void CTaskOpPosNoGravity::reset()
  {
    data_ = S_NULL;
    dynamics_ = S_NULL;
    has_been_init_ = false;
...
bool CTaskOpPosNoGravity::computeServo(const SRobotSensorData* arg_sensors)
{
#ifdef DEBUG
  ...
  if(data_->has_been_init_)
  {
    ...
    //NOTE : This does not compensate for gravity.
...
bool CTaskOpPosNoGravity::computeModel()
{
  ...
  if(data_->has_been_init_)
  {
    ...
    if(!lambda_inv_singular_)
    {
      ...
      {
        std::cout<<"\nCTaskOpPosNoGravity::computeModel() : Warning. Lambda_inv is rank deficient. Using svd. Rank = "<<qr_.rank();
...
bool CTaskOpPosNoGravity::achievedGoalPos()
{
  sFloat dist;
