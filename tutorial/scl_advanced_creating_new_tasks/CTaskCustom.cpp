...
namespace scl_app
{
  ...
  // Inherited stuff
  //************************
  bool CTaskCustom::init(STaskBase* arg_task_data,
      CDynamicsBase* arg_dynamics)
  {
    try
    {
      ...
      data_ = dynamic_cast<STaskCustom*>(arg_task_data);
    ...
    {
      std::cerr<<"\nCTaskCustom::init() :"<<e.what();
      has_been_init_ = false;
    }
    return has_been_init_;
  }

  STaskBase* CTaskCustom::getTaskData()
  { return data_; }

  /** Sets the current goal position */
  bool CTaskCustom::setGoalPos(const Eigen::VectorXd & arg_goal)
  {
    ...
    {
      std::cerr<<"\nCTaskCustom::setGoalPos() : Error : Goal vector's size != data_->dof_task_"<<std::flush;
      assert(false);
    }
#endif
    return false;
  ...
  bool CTaskCustom::setGoalVel(const Eigen::VectorXd & arg_goal)
  {
    if((data_->dof_task_ == arg_goal.cols() && 1 == arg_goal.rows()) ||
        (1 == arg_goal.cols() && data_->dof_task_ == arg_goal.rows()) )
    {
    ...
    {
      std::cerr<<"\nCTaskCustom::setGoalVel() : Error : Goal vector's size != data_->dof_task_"<<std::flush;
      assert(false);
    }
#endif
    return false;
  ...
  bool CTaskCustom::setGoalAcc(const Eigen::VectorXd & arg_goal)
  {
    if((data_->dof_task_ == arg_goal.cols() && 1 == arg_goal.rows()) ||
        (1 == arg_goal.cols() && data_->dof_task_ == arg_goal.rows()) )
    {
    ...
    {
      std::cerr<<"\nCTaskCustom::setGoalAcc() : Error : Goal vector's size != data_->dof_task_"<<std::flush;
      assert(false);
    }
#endif
    return false;
  ...
  bool CTaskCustom::computeServo(const SRobotSensors* arg_sensors)
  {
#ifdef DEBUG
  ...
  bool CTaskCustom::computeModel(const scl::SRobotSensors* arg_sensors)
  {
#ifdef DEBUG
    assert(has_been_init_);
  ...
  bool CTaskCustom::achievedGoalPos()
  {
    sFloat dist;
  ...
  scl::sBool registerType_TaskCustom()
  {
    ...
    try
    {
      sutil::CDynamicType<std::string,scl_app::CTaskCustom> typeCTaskCustom(std::string("CTaskCustom"));
      flag = typeCTaskCustom.registerType();
      if(false == flag) {throw(std::runtime_error("Could not register type CTaskCustom"));}

      sutil::CDynamicType<std::string,scl_app::STaskCustom> typeSTaskCustom(std::string("STaskCustom"));
      flag = typeSTaskCustom.registerType();
      if(false == flag) {throw(std::runtime_error("Could not register type STaskCustom"));}
